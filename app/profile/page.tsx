...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/auth/oauth/linked").then(r => r.ok ? r.json() : { linkedProviders: [] }),
    ]).then(([userData, oauthData]) => {
      if (cancelled) return;
      if (!userData) { router.push("/login?redirect=/profile"); return; }
      setUser(userData);
      setName(userData.name);
      setEmail(userData.email);
      setLinkedProviders(oauthData.linkedProviders || []);
    })
                ...
                {["google", "discord"].map((provider) => {
                  ...
                  return (
                    <button
                      key={provider}
                      onClick={() => {
                        if (!linked) router.push("/api/auth/oauth/" + provider);
                      }}
                      disabled={linked}
                      className={`flex items-center justify-between w-full px-4 py-2.5 border rounded-xl transition-all ${
                        linked
                          ? "border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 cursor-default"
                          : "border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300 capitalize">{provider}</span>
                      </div>
                      <span className={`text-xs font-medium ${linked ? "text-emerald-600 dark:text-emerald-400" : "text-indigo-600 dark:text-indigo-400"}`}>
                        {linked ? "Linked" : "Link"}
                      </span>
                    </button>
                  );
